...
def calculate_ranks(out_path_all, smiles_str, ligand_count):
    file = f'{ligand_count}M_all.out'
    results = []
    ...
    df_results['top_percent'] = (df_results['rank'] / (1000000 * ligand_count + 1)) * 100
...
def duplicate_smiles(smiles_path, multiplier):
    """Duplicate the SMILES entries to multiply the number of ligands."""
    with open(smiles_path, 'r') as file:
        smiles_lines = file.readlines()

    # Duplicate the content based on the multiplier
    duplicated_smiles = smiles_lines * multiplier

    # Create a new file path for the multiplied ligands
    new_smiles_path = smiles_path.replace('1M', f'{multiplier}M')
    
    with open(new_smiles_path, 'w') as file:
        file.writelines(duplicated_smiles)

    return new_smiles_path

def log_time(start_time, end_time, base_path, multiplier):
    runtime = end_time - start_time
    log_path = os.path.join(base_path, "runtime_log.csv")
    with open(log_path, 'a') as log_file:
        log_file.write(f"{multiplier}M,{runtime}\n")

def main(pdbid, multiplier):
    base_path = f"case_study_{pdbid}"
    cif_path = f"./{base_path}/{pdbid}.cif"
    residue_list_all = get_residue_list(cif_path)
    
    ligands_path = f"{base_path}/Chemspace_Screening_Compounds_SMILES_sampled1M.smi"
    
    # Duplicate SMILES file to create a multiplied ligands set
    multiplied_smiles_path = duplicate_smiles(ligands_path, multiplier)
    
    # Run inference on the multiplied SMILES set
    out_path_all = f"{base_path}/{multiplier}M_all.out"
    
    start_time = time.time()
    do_inference(
        cif_path=cif_path,
        residue_list=residue_list_all.split(","),
        ligands_path=multiplied_smiles_path,
        out_path=out_path_all,
        dump_all=True,
    )
    
    with open(multiplied_smiles_path, 'r') as f:
        content = f.readlines()
        smiles_str = content[-1].strip()
    
    results_df = calculate_ranks(out_path_all, smiles_str, multiplier)
    results_df.to_csv(f"{base_path}/results_{multiplier}M.csv")
    
    end_time = time.time()
    log_time(start_time, end_time, base_path, multiplier)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run inference on specified PDB ID with multiplied SMILES ligand set.")
    parser.add_argument("--pdbid", required=True, help="The PDB ID for the case study (e.g., 9cpd).")
    parser.add_argument("--multiplier", type=int, required=False, default=2, help="Factor by which to multiply the SMILES ligands (e.g., 2 for double, 3 for triple).")
    
    args = parser.parse_args()
    main(args.pdbid, args.multiplier)
